...
import { QueryDataCities, useQueryCitiesData } from "@/hooks/useQueryCitiesData"
...
export function DataBarChart({data} :DataBarChartProps) {

  const total = React.useMemo(
    () => ({
      desktop: data?.reduce((acc, curr) => acc + curr.value, 0) ?? 0,
    }),
    [data]
  )
